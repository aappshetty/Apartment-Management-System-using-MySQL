...
def generate_rent_alerts(connection, threshold_amount):
    try:
        ...
        # Use a nested query to get the relevant information
        cursor.execute(
            """
            INSERT INTO RentAlerts (ApartmentID, TotalDueAmount, AlertDate)
            SELECT ApartmentID, TotalDueAmount, CURDATE()
            FROM (
        ...
        # Fetch the inserted alerts
        cursor.execute(
            """
            SELECT ApartmentID, TotalDueAmount, AlertDate
            FROM RentAlerts
            WHERE AlertDate = CURDATE()
            """
        )
        inserted_alerts = cursor.fetchall()
...
# Example call to the function (Replace create_connection with your actual connection function)
...
def display_maintenance_requests_ui(maintenance_requests):
    st.subheader("Maintenance Request Entries")
